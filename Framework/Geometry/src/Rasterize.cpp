...
void Raster::resize(size_t numVolumeElements) {
  m_L1s.resize(numVolumeElements);
  m_elementVolumes.resize(numVolumeElements);
  m_elementPositions.resize(numVolumeElements);
}
  ...
  for (size_t i = 0; i < numSlices; ++i) {
    ...
    for (size_t j = 0; j < numAnnuli; ++j) {
      ...
      for (size_t k = 0; k < Ni; ++k) {
        ...
        result.m_elementPositions[counter](R * sin(phi), z, R * cos(phi));
        assert(shape->isValid(m_elementPositions[counter]));
        // Create track for distance in cylinder before scattering point
        Track incoming(result.m_elementPositions[counter],
                       beamDirection * -1.0);

        shape.interceptSurface(incoming);
        result.m_L1s[counter] = incoming.cbegin()->distFromStart;
        ...
        result.m_elementVolumes[counter] = elementVolume;
